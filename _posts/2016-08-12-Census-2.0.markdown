...
One of the biggest issues (if not the biggest) with the existing Census setups was the inability to cope with the demand requirements of 5million+ households having dinner and then all firing up their browsers within 45 minutes of each other.
...
The function would only need to validate a relatively small data packet for completeness and data validity, on failure return sufficient error messages for the client side application to give feedback to the user.
...
Yes, there are real-world requirements that this simple overview doesn't cover; in-progress saving of forms, clients that cannot (or will not) use
Javascript, and probably more. This is a thought exercise rather than a full-fledged system design -- however, if you'd _like_ a full-fledged system
